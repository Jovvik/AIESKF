...
class LC_est_KG(nn.Module):
    def __init__(
        ...
        self.rnn = {"gru": nn.GRU, "lstm": nn.LSTM, "rnn": nn.RNN}[recurrent_kind](
            self.input_dim + cnn_out_channels * (Fs - 1), self.hidden_dim, self.n_layers, batch_first=True
        )
    ...
    def init_hidden_state(self, batch_size):
        if self.recurrent_kind == "lstm":
            state1 = torch.zeros(self.n_layers, batch_size, self.hidden_dim).to(self.dev)
            state2 = torch.zeros(self.n_layers, batch_size, self.hidden_dim).to(self.dev)
    ...
    def get_KG(self, features_KG, imu, dr):
        ...
        # imu_acc_diff = imu[:,1:,:3] 
        # imu_gyro_diff = imu[:,1:,3:] 
        ...
        # dr_imu = torch.cat([imu_acc_diff,imu_gyro_diff],dim=2)
        # dr_diff = dr
        # print(dr_diff.shape)
        cnn_out = self.cnn(torch.transpose(dr_imu, 1, 2))
        ...
        out = self.outputlayer(self.leakyrelu(Dropout_out))
    ...
    def forward(self, meas, dr_temp, est_C_b_e_old, imu, prev_out, dr):
        ...
        if self.idx_feedback_type == 'pvab':
            ...
            #     ]
            # )
            skew_att = torch.zeros(Inno_att_scale.shape[0],3,3).to(self.dev)
