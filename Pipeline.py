...
class Pipeline_LC:
    ...
    def test_lc(self, test_loader, test_dataset, Fs, Fs_meas):
        ...
        predict_KG_net = torch.zeros((test_dataset.num_traj, test_dataset.time_step, 9 , 6))
    ...
    def GnssInsLooseCoupling(self, datatype, test_loader, test_dataset, Fs, Fs_meas):
        ...
        if  datatype == 'bias' :
            accel_noise_PSD_x = 100 * functions.micro_g_to_meters_per_second_squared ** 2
            accel_noise_PSD_y = 100 * functions.micro_g_to_meters_per_second_squared ** 2
            accel_noise_PSD_z = 100 * functions.micro_g_to_meters_per_second_squared ** 2
            # Acc bias variation
            # (m^2 s^-5)
            ...
            # Gyro bias variation
            # (rad^2 s^-3)
            gyro_bias_PSD_x = 4e-11
            gyro_bias_PSD_y = 4e-11
            gyro_bias_PSD_z = 4e-11
        ...
        init_att_unc_x = (2 * 3.14/180) 
        init_att_unc_y = (2 * 3.14/180) 
        init_att_unc_z = (2 * 3.14/180) 
        # Initial accelerometer bias uncertainty per instrument (micro-g, converted to m/s^2)
        init_b_a_unc_x = 0.002 
        init_b_a_unc_y = 0.002 
        init_b_a_unc_z = 0.002 
        # Initial gyro bias uncertainty per instrument (deg/hour, converted to rad/sec)
        init_b_g_unc_x = 0.0002 
        init_b_g_unc_y = 0.0002 
        init_b_g_unc_z = 0.0002 
        ...
        ref_traj = torch.zeros((test_dataset.num_traj, test_dataset.time_step * Fs, 33))
        KG_MB = torch.zeros((test_dataset.num_traj, test_dataset.time_step, 15, 6))
        ...
        return predict_traj, ref_traj, bias_history, predict_traj_llh, ref_traj_llh, P_est, KG_MB
