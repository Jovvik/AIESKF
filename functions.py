...
def skew_symmetric_gpu(vec):
    ...
    #   A       3x3matrix
    Matr = torch.stack((
        torch.zeros(1).squeeze().to(dev), -vec[2], vec[1],
        vec[2], torch.zeros(1).squeeze().to(dev), -vec[0],
        -vec[1], vec[0], torch.zeros(1).squeeze().to(dev)
    )).reshape(3, 3)
    return Matr
...
def get_R_e_n(cos_lat, sin_lat, cos_lon, sin_lon):
    return torch.stack(
        (
            ...
            -sin_lon, cos_lon, torch.zeros(1).squeeze().to(dev),
...
def get_R_e_n_0(cos_lat, sin_lat, cos_lon, sin_lon):
    return torch.stack(
        (
            -sin_lat * cos_lon, -sin_lat * sin_lon, cos_lat,
...
def geo_ned2ecef_0(*arg):
    ...
    # Calculate ECEF to NED coordinate transformation matrix
    # todo: Maybe we should .detach() here
    R_e_n = get_R_e_n_0(cos_lat, sin_lat, cos_lon, sin_lon)
    R_n_e = R_e_n.T
    # Transform velocity using (2.73)
    v_eb_e = R_n_e @ arg[1]
    if len(arg) == 2:
...
def Model_LC(meas, dr_temp, est_C_b_e_old, imu, imu_error, P_old, Q, R, Fs_meas):
    ...
    return dr_new, est_C_b_e_new, imu_error_new, P_new, K
...
def lc_propagate_f(est_pos_old, est_C_b_e_old, imu, Fs_meas):
    ...
    F22 = torch.eye(3).to(dev) - 2 * OMEGA_ie_e * dt
    F23 = -dt * skew_symmetric_gpu((est_C_b_e_old @ meas_f_ib_b).squeeze())
    row1 = torch.cat((F12, torch.zeros((3, 6))), dim=1).to(dev)
    row2 = torch.cat((F21, F22, F23), dim=1).to(dev)
    row3 = torch.cat((torch.zeros((3, 6)), torch.eye(3)), dim=1).to(dev)
...
def KG2COV(KG_matrix, H):
    H_tilde = torch.linalg.inv(H.T @ H)
    I_m = torch.eye(9)
    I_n = torch.eye(6)
    Cov = torch.zeros((KG_matrix.shape[0], KG_matrix.shape[1],9,9))
    for i in range(KG_matrix.shape[0]):
        for j in range(KG_matrix.shape[1]):
            Cov[i,j] = (I_m - KG_matrix[i,j]) @ H_tilde @ H.T @ torch.linalg.inv(I_n - H @ KG_matrix[i,j]) @ H @ KG_matrix[i,j] @ H @ H_tilde
    return Cov
